...
if ( ! function_exists( 'get_product_data' )) {
    function get_product_data($product, $productId) {
        ...
        $productData['shortDesc'] = isset($product['description']) ? $product['description'] : '';
...
if (! function_exists( 'get_np_shop_categories_html' )) {
    ...
    function get_np_shop_categories_html($args) {
        $categories_html = '';
        $args['category_id'] = isset($_GET['category_id']) ? $_GET['category_id'] : 0;
        $category = npGetCategories($args['category_id']);
        ...
        $categoryCheck = isset($_GET['category_id']) && $_GET['category_id'] ? true : false;
        if ($args['category_id']) {
            if ($categoryCheck) {
                // add back link
                $linkHref = $category['categoryId'] ? home_url('?products-list&category_id=' . $category['categoryId']) : home_url('?products-list');
            ...
            if ($args['type'] === 'vertical') {
                $template_cats = array(0 => npGetCategories($args['category_id']));
                $childs = getChildCategories($args['category_id'], $template_cats);
                $subCats_html = getShopSubCatHtml($childs, $args);
            }
            // add current cat link
            $needShowIcon = is_np_category_has_child($category);
            $categories_html .= str_replace(
                array($linkTitle, $linkUrl, $isActiveLi, $isActiveLink, $iconOpen, $liOpen, $showIcon, '</li>'),
                array($category['title'], home_url('?products-list&category_id=' . $category['id']), 'u-active', 'active', $iconClosed, $liClosed, $needShowIcon,  $subCats_html . '</li>'),
                $args['itemTemplate']
            );
            if ($args['type'] === 'horizontal') {
                $template_cats = array(0 => npGetCategories($args['category_id']));
                $childs = getChildCategories($args['category_id'], $template_cats);
                $categories_html .= getShopSubCatHtml($childs, $args, true);
            }
        } else {
            $template_cats = npGetCategories($args['category_id']);
            ...
            if($template_cats) {
                foreach($template_cats as $template_category) {
                    ...
                    $categories_html .= str_replace(
                        ...
                        array($template_category['title'], home_url('?products-list&category_id=' . $template_category['id']), '', '', $needShowIcon, $iconClosed, $liClosed, 'u-expand-closed', $subCats_html . '</li>'),
                        $args['itemTemplate']
                    );
                }
            }
    ...
    function getShopSubCatHtml($categories, $args, $onlyItems=false) {
        ...
        if ($args['type'] === 'horizontal') {
            foreach ($categories as $category) {
                if ($args['category_id'] === $category['categoryId']) {
                    $output .= str_replace(
                        array('{content}', '{url}', '{activeLi}', '{activeLink}', 'fill-opacity="1"', 'u-root'),
                        array($category['title'], home_url('?products-list&category_id=' . $category['id']), '', '', 'fill-opacity="0"', ''),
                        $args['itemTemplate']
                    );
            ...
            foreach ($categories as $category) {
                ...
                $output .= str_replace(
                    ...
                    array($category['title'], home_url('?products-list&category_id=' . $category['id']), '', ''),
                    $args['itemTemplate']
                );
                if (!empty($category['items'])) {
...
if (!function_exists('npGetCategories')) {
    function npGetCategories($category_id) {
        $allCategories = np_get_all_categories();
        $result = buildCategoryTree($allCategories);
        if ($category_id) {
            $result = getCategoryById($category_id, $result);
        }
        return $result;
    }
}
...
function getCategoryById($categoryId, $categories) {
    foreach ($categories as $category) {
        ...
        if (isset($category['items'])) {
            $result = getCategoryById($categoryId, $category['items']);
            if ($result !== null) {
                return $result;
            }
        }
...
if (! function_exists('get_np_shop_categories_filter_html')) {
    ...
    function get_np_shop_categories_filter_html($args) {
        ...
        // add item all
        $categories_filter_html .= strtr($args['itemTemplate'], array('{categories_filters_content}' => __('All', 'yxist_mvp'), '{categories_filters_value}' => ''));
        // add item featured
        $featuredLink = home_url('?products-list&category_id=featured');
        $item = strtr($args['itemTemplate'], array('{categories_filters_content}' => __('Featured', 'yxist_mvp'), '{categories_filters_value}' => 'featured'));
        if ( 'featured' === $args['category_id'] ) {
    ...
    function generate_np_category_options( $categories, $args, $parent = 0, $prefix = '' ) {
        ...
        foreach ( $categories as $category ) {
            if ( $category['categoryId'] == $parent ) {
                $link = home_url('?products-list&category_id=' . $category['id']);
